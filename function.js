...
const quizInfo = document.getElementById("head");
  ...
  if (answer) {
    ...
    if (currentQuiz < quizData.length) {
      ...
      quiz.innerHTML = '<div class="result-container">' +
        ...
        `<p class="result-text">You Scored: <strong>${score}/${quizData.length}</strong></p>` +
        `<p class="result-text">Your Percentage: <strong>${percentage}%</strong></p>` +
        '<button id="reload" onclick="location.reload()">Reload Quiz</button>' +
        '</div>';
    }
  } else {
    alert("Please select an answer before proceeding.");
    // or add other logic to handle the case when no answer is selected
  }
});
